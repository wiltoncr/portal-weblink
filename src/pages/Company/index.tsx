...
const Company = () => {
  const [companyData, setCompanyData] = useState<CompanyData>({ companys: [] });
  ...
  useEffect(() => {
    const fetchCompanyData = async () => {
      try {
        ...
        const response: CompanyData = await companyService.getAllCompany(token);

        if (response.companys.length > 0) {
          setCompanyData(response);
        }
      } catch (error) {
        console.error('Erro ao buscar informações do company:', error);
      }
    };
    fetchCompanyData();
  }, []);

  const sortCompanyByName = () => {
    if (companyData) {
      const sortedCompany = companyData;

      if (sortOrder === 'asc') {
        sortedCompany.companys.sort((a, b) => a.name.localeCompare(b.name));
        setSortOrder('desc');
      } else {
        sortedCompany.companys.sort((a, b) => b.name.localeCompare(a.name));
        setSortOrder('asc');
      }

      setCompanyData(sortedCompany);
    } else {
      setCompanyData((prevState) => ({
        ...prevState,
        companys: [...(prevState.companys ?? [])],
      }));
    }
  };
  ...
  const filteredCompany = companyData.companys.filter((item) => {
        ...
        <div className="Layout Layout--flowRow-until-md Layout--sidebarPosition-start Layout--sidebarPosition-flowRow-start">
          <div className="Layout-main">
            <div>
              <div className="border-bottom color-border-muted py-3">
                ...
                <div className="d-flex flex-items-start">
                    ...
                    <div className="d-flex flex-column flex-lg-row flex-auto">
                      ...
                      <div className="d-flex flex-wrap gap-2">
                        <div className="details-reset details-overlay position-relative mt-1 mt-lg-0">
                          <div className="btn" onClick={() => sortCompanyByName()}>
                            <span>Name</span>
                          </div>
                        </div>
                        <div className="details-reset details-overlay position-relative mt-1 mt-lg-0">
                  ...
                  <div className="d-none d-md-flex flex-md-items-center flex-md-justify-end">
                    <Link to={`/company/add`} className="text-center btn btn-primary ml-2">
                      <svg
                        aria-hidden="true"
                        height="16"
                        viewBox="0 0 16 16"
              ...
              {companyData.companys.length === 0 ? (
                <p>Carregando...</p>
              ) : (
                <div className="list">
                  <ul>
                    {filteredCompany?.map((compantyItem) => (
                      <li
                        key={compantyItem.id}
