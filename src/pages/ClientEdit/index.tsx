...
const ClientEdit = () => {
  ...
  const handleDeleteClient = async () => {
    try {
      ...
      const response: ClientData = await clientService.delAccess(Number(id), token);
      if (response) {
  ...
  const handleSaveClient = async () => {
    try {
      ...
      const response: ClientData | null = await clientService.putClient(clientsData, token);
      if (response) {
        console.log(response);
      }
    } catch (error) {
  ...
  useEffect(() => {
    const fetchClientDataById = async () => {
      try {
        ...
        const clientResponse: ClientData | null = await clientService.getByIdClient(id, token);
            ...
            <div className="Box d-flex flex-column Box-overlay--wide">
              <div className="width-full">
                <div className="Box-header">
                  <h1 className="Box-title">Editando Cliente</h1>
                </div>
