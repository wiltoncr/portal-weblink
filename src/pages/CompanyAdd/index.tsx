...
const CompanyAdd = () => {
  ...
  const [companysData, setCompanysData] = useState<CompanyData>({
    companys: [{ id: 0, name: '', cnpj: '', email: '' }],
  });
  ...
  const handleSaveCompany = async () => {
    try {
      ...
      const response: CompanyData = await companyService.postCompany(companysData, token);
      if (response) {
          ...
          <div className="Box d-flex flex-column Box-overlay--wide">
            <div className="width-full">
              <div className="Box-header">
                <h1 className="Box-title">Adicionando Empresa</h1>
              </div>
